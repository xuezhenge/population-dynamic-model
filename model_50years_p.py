...
def batch(a,w,a_20,w_20,TminA,TmaxA, TminL,TmaxL,export_fns):
    tic = time.time()
    export_folder, plotAnp_folder,plotAp_folder, plotL_folder, plotAL_folder = export_fns   
    
    #output file name
    if alter == 'aw_real':
        temp_fn = "a_{}_w_{}.csv".format(a_20,w_20)
    else:
    ...
    if Temp_max <= Tmin_min or Temp_min >= Tmax_max:
        ...
        def year_loop(A_add,L_add,ov_surv=1):
...
def batch20_80(i,TminA,TmaxA,TminL,TmaxL,export_fns_2080):
    df = pd.read_csv('../../inputs/a_w_changes_2020_2080_1160.csv')
    ...
    a_80 = a_20 + a_change_
    w_80 = w_20 + w_change_
    batch(a_80,w_80,a_20,w_20,TminA,TmaxA,TminL,TmaxL,export_fns_2080)

def folders(year):
    fold_dir = f"../../outputs/exports_case{case}_{alter}_50years_p"
    export_fns = []
