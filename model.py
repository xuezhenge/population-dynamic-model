...
parser.add_argument("--case", type=int, 
    default="1", help="1 or 2 or 3 or 4")
...
parser.add_argument('--A_add', type=int,
    default=100000, help = "Initial value of aphid population")
...
def batch(a,w,export_fns,year):
    tic = time.time()
    # pt_Tmin_maxs_sel = pt_Tmin_maxs[args.start_idx:args.end_idx]
    # pt_Tmin_max = pt_Tmin_maxs_sel[idx]
    # idx_pt = pt_Tmin_max[0]
    # TminA = pt_Tmin_max[1]
    # TmaxA = pt_Tmin_max[2]
    # temps = data_temps[idx_pt].copy()
    # lat = lats[idx_pt].copy()
    # lon = lons[idx_pt].copy()
    # print('lat = {} lon = {} Tmin = {} Tmax = {}'.format(lat,lon,TminA,TmaxA))
        ...
        for i in range(1, n_t):
            ...
            if TminH > TminA:
                if num_change_A == 0 and num_change_H == 0:
                    Aden_t = A_add; Aap_t = A_add/5; A1_t = Aap_t; A2_t = Aap_t;A3_t = Aap_t;A4_t = Aap_t
                    Hden_t = 0; Hegg_t = 0; H1_t = 0; H2_t = 0; H3_t = 0; H4_t = 0; Hpupa_t = 0; Hf_t = 0; Hm_t = 0
                elif num_change_A == 1 and num_change_H == 0:
                    Aden_t = Aden[i-1];Aap_t = Aap[i-1]; A1_t = A1[i-1]; A2_t = A2[i-1]; A3_t = A3[i-1]; A4_t = A4[i-1]
                    Hden_t = A_add/Ratio; Hegg_t = Hden_t/8; H1_t = Hegg_t; H2_t = Hegg_t; H3_t = Hegg_t; H4_t = Hegg_t; Hpupa_t = Hegg_t; Hf_t = Hegg_t; Hm_t = Hegg_t
            ...
            # set start condition
            if TminH > TminA:
                if Aden[i] <= Aden_t and num_change_A == 0:
                    Aden[i] = 0; Aap[i] = 0; A1[i] = 0; A2[i] = 0; A3[i] = 0; A4[i] = 0;Adeath[i] = 0
                else:
                    num_change_A = 1
                if Hden[i] <= Hden_t and num_change_A == 1:
                    Hden[i] = 0; Hegg[i] = 0; H1[i] = 0; H2[i] = 0; H3[i] = 0; H4[i] = 0; Hpupa[i] = 0; Hf[i] = 0; Hm[i] = 0;Predated_prey[i] = 0
                elif Hden[i] > Hden_t and num_change_A == 1:
                    num_change_H = 1
            elif TminH <= TminA:
                if Aden[i] <= Aden_t and num_change_A == 0:
                    Aden[i] = 0; Aap[i] = 0; A1[i] = 0; A2[i] = 0; A3[i] = 0; A4[i] = 0;Adeath[i] = 0
                else: num_change_A = 1
                if num_change_A == 1 and num_change_H == 0:
                    Aden[i] = A_add; Aap[i] = A_add/5; A1[i] = A_add/5; A2[i] = A_add/5; A3[i] = A_add/5; A4[i] = A_add/5; Adeath[i] = 0
                    Hden[i] = A_add/Ratio; Hegg[i] = Hden[i]/8; H1[i] = Hegg[i]; H2[i] = Hegg[i]; H3[i] = Hegg[i]; H4[i] = Hegg[i]; Hpupa[i] = Hegg[i]; Hf[i] = Hegg[i]; Hm[i] = Hegg[i];Predated_prey[i] = Hegg[i]
                    num_change_H = 1
